...
def test_serve_rejects_multi_worker_in_aggregator_mode(monkeypatch):
    # type: (pytest.MonkeyPatch) -> None
    """Guard raises typer.Exit when --workers > 1 is combined with aggregator mode (any backend)."""
    monkeypatch.setattr(uvicorn, "run", lambda **_kwargs: None)
    monkeypatch.setattr(search_opts, "index_uri", "lmdb:///tmp/test-index")
    monkeypatch.setattr(search_opts, "aggregator_network", "testnet")
...
def test_serve_allows_single_worker_in_aggregator_mode(monkeypatch):
    # type: (pytest.MonkeyPatch) -> None
    """Guard does not fire for a single worker in aggregator mode."""
    called = {}
