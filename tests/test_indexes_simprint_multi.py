"""Tests for SimprintMultiIndex backend-agnostic multi-type simprint index."""

import json

import pytest

from iscc_search.indexes.simprint.multi import SimprintMultiIndex
from iscc_search.indexes.simprint.models import SimprintEntryMulti, SimprintRaw


@pytest.fixture
def temp_index_path(tmp_path):
    # type: (Path) -> Path
    """Provide a temporary directory for index storage."""
    return tmp_path / "multi_index"


@pytest.fixture
def sample_entries():
    # type: () -> list[SimprintEntryMulti]
    """Create sample multi-type entries for testing."""
    realm_id = b"\x00\x10"  # ISCC-ID header

    entries = [
        SimprintEntryMulti(
            iscc_id=realm_id + b"\x12\x34\x56\x78\x9a\xbc\xde\xf0",
            simprints={
                "CONTENT_TEXT_V0": [
                    SimprintRaw(simprint=b"\x01" * 8, offset=0, size=512),
                    SimprintRaw(simprint=b"\x02" * 8, offset=512, size=489),
                ],
                "SEMANTIC_TEXT_V0": [
                    SimprintRaw(simprint=b"\x03" * 8, offset=0, size=1024),
                ],
            },
        ),
        SimprintEntryMulti(
            iscc_id=realm_id + b"\xfe\xdc\xba\x98\x76\x54\x32\x10",
            simprints={
                "CONTENT_TEXT_V0": [
                    SimprintRaw(simprint=b"\x01" * 8, offset=0, size=256),  # Same simprint
                ],
                "SEMANTIC_TEXT_V0": [
                    SimprintRaw(simprint=b"\x04" * 8, offset=0, size=512),
                ],
            },
        ),
    ]
    return entries


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_init_creates_directory(temp_index_path, backend):
    # type: (Path, str) -> None
    """Test that __init__ creates the index directory."""
    index = SimprintMultiIndex(str(temp_index_path), backend=backend)
    assert temp_index_path.exists()
    assert temp_index_path.is_dir()
    index.close()


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_init_with_file_uri(temp_index_path, backend):
    # type: (Path, str) -> None
    """Test initialization with file:// URI."""
    uri = f"file://{temp_index_path}"
    index = SimprintMultiIndex(uri, backend=backend)
    assert temp_index_path.exists()
    index.close()


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_init_with_backend_uri(temp_index_path, backend):
    # type: (Path, str) -> None
    """Test initialization with backend-specific URI."""
    uri = f"{backend}://{temp_index_path}"
    index = SimprintMultiIndex(uri, backend=backend)
    assert temp_index_path.exists()
    index.close()


def test_init_with_invalid_backend(temp_index_path):
    # type: (Path) -> None
    """Test initialization with invalid backend raises error."""
    with pytest.raises(ValueError, match="Unknown backend"):
        SimprintMultiIndex(str(temp_index_path), backend="invalid")


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_add_raw_multi_creates_type_indexes(temp_index_path, sample_entries, backend):
    # type: (Path, list[SimprintEntryMulti], str) -> None
    """Test that add_raw_multi creates type-specific files/directories."""
    index = SimprintMultiIndex(str(temp_index_path), backend=backend)
    index.add_raw_multi(sample_entries)

    # Verify backend-specific file structure
    if backend == "lmdb":
        # LMDB uses .lmdb files
        assert (temp_index_path / "SIMPRINT_CONTENT_TEXT_V0.lmdb").exists()
        assert (temp_index_path / "SIMPRINT_SEMANTIC_TEXT_V0.lmdb").exists()
        # Lock files should also exist
        assert (temp_index_path / "SIMPRINT_CONTENT_TEXT_V0.lmdb-lock").exists()
        assert (temp_index_path / "SIMPRINT_SEMANTIC_TEXT_V0.lmdb-lock").exists()
    elif backend == "lancedb":
        # LanceDB uses directories
        assert (temp_index_path / "SIMPRINT_CONTENT_TEXT_V0").is_dir()
        assert (temp_index_path / "SIMPRINT_SEMANTIC_TEXT_V0").is_dir()

    index.close()


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_add_raw_multi_extracts_realm_id(temp_index_path, sample_entries, backend):
    # type: (Path, list[SimprintEntryMulti], str) -> None
    """Test realm_id extraction from first entry and propagation to sub-indexes."""
    index = SimprintMultiIndex(str(temp_index_path), backend=backend)
    index.add_raw_multi(sample_entries)

    assert index.realm_id == b"\x00\x10"

    # Verify realm_id propagated to sub-indexes
    assert "CONTENT_TEXT_V0" in index.indexes
    assert "SEMANTIC_TEXT_V0" in index.indexes
    assert index.indexes["CONTENT_TEXT_V0"].realm_id == b"\x00\x10"
    assert index.indexes["SEMANTIC_TEXT_V0"].realm_id == b"\x00\x10"

    index.close()


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_add_raw_multi_validates_realm_id(temp_index_path, sample_entries, backend):
    # type: (Path, list[SimprintEntryMulti], str) -> None
    """Test that mismatched realm_id raises error."""
    index = SimprintMultiIndex(str(temp_index_path), backend=backend)
    index.add_raw_multi([sample_entries[0]])

    # Try to add entry with different realm_id
    bad_entry = SimprintEntryMulti(
        iscc_id=b"\xff\xff\x12\x34\x56\x78\x9a\xbc\xde\xf0",
        simprints={"CONTENT_TEXT_V0": [SimprintRaw(simprint=b"\x05" * 8, offset=0, size=100)]},
    )

    with pytest.raises(ValueError, match="realm mismatch"):
        index.add_raw_multi([bad_entry])

    index.close()


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_add_raw_multi_invalid_iscc_id_length(temp_index_path, backend):
    # type: (Path, str) -> None
    """Test that invalid ISCC-ID length raises error."""
    index = SimprintMultiIndex(str(temp_index_path), backend=backend)

    bad_entry = SimprintEntryMulti(
        iscc_id=b"\x00\x10\x12",  # Only 3 bytes instead of 10
        simprints={"CONTENT_TEXT_V0": [SimprintRaw(simprint=b"\x01" * 8, offset=0, size=100)]},
    )

    with pytest.raises(ValueError, match="must be 10 bytes"):
        index.add_raw_multi([bad_entry])

    index.close()


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_add_raw_multi_empty_list(temp_index_path, backend):
    # type: (Path, str) -> None
    """Test that empty entry list is handled gracefully."""
    index = SimprintMultiIndex(str(temp_index_path), backend=backend)
    index.add_raw_multi([])

    assert index.realm_id is None
    assert len(index.indexes) == 0
    index.close()


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_add_raw_multi_duplicate_entries(temp_index_path, sample_entries, backend):
    # type: (Path, list[SimprintEntryMulti], str) -> None
    """Test backend-specific add semantics."""
    index = SimprintMultiIndex(str(temp_index_path), backend=backend)

    # Add same entries twice
    index.add_raw_multi(sample_entries)
    index.add_raw_multi(sample_entries)

    # Verify based on backend semantics
    content_index = index.indexes["CONTENT_TEXT_V0"]
    if backend == "lmdb":
        # LMDB has add-once semantics
        assert len(content_index) == 2  # Both unique ISCC-IDs indexed
    elif backend == "lancedb":
        # LanceDB has append semantics
        assert len(content_index) == 2  # Still 2 unique ISCC-IDs
    index.close()


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_search_raw_multi_single_type(temp_index_path, sample_entries, backend):
    # type: (Path, list[SimprintEntryMulti], str) -> None
    """Test search with single simprint type."""
    index = SimprintMultiIndex(str(temp_index_path), backend=backend)
    index.add_raw_multi(sample_entries)

    # Search for simprint that appears in both assets
    results = index.search_raw_multi(
        {"CONTENT_TEXT_V0": [b"\x01" * 8]},
        limit=10,
        threshold=0.0,
        detailed=True,
    )

    assert len(results) == 2
    assert results[0].iscc_id in [e.iscc_id for e in sample_entries]
    assert "CONTENT_TEXT_V0" in results[0].types
    index.close()


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_search_raw_multi_multiple_types(temp_index_path, sample_entries, backend):
    # type: (Path, list[SimprintEntryMulti], str) -> None
    """Test search across multiple simprint types."""
    index = SimprintMultiIndex(str(temp_index_path), backend=backend)
    index.add_raw_multi(sample_entries)

    # Search both types
    results = index.search_raw_multi(
        {
            "CONTENT_TEXT_V0": [b"\x01" * 8],
            "SEMANTIC_TEXT_V0": [b"\x03" * 8],
        },
        limit=10,
        threshold=0.0,
        detailed=True,
    )

    assert len(results) >= 1
    # First result should match both types
    top_match = results[0]
    assert len(top_match.types) >= 1
    index.close()


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_search_raw_multi_empty_query(temp_index_path, sample_entries, backend):
    # type: (Path, list[SimprintEntryMulti], str) -> None
    """Test search with empty query."""
    index = SimprintMultiIndex(str(temp_index_path), backend=backend)
    index.add_raw_multi(sample_entries)

    results = index.search_raw_multi({}, limit=10, threshold=0.8, detailed=True)
    assert results == []
    index.close()


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_search_raw_multi_unknown_type(temp_index_path, sample_entries, backend):
    # type: (Path, list[SimprintEntryMulti], str) -> None
    """Test search with type that hasn't been indexed."""
    index = SimprintMultiIndex(str(temp_index_path), backend=backend)
    index.add_raw_multi(sample_entries)

    results = index.search_raw_multi(
        {"UNKNOWN_TYPE_V0": [b"\x01" * 8]},
        limit=10,
        threshold=0.8,
        detailed=True,
    )
    assert results == []
    index.close()


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_search_raw_multi_detailed_false(temp_index_path, sample_entries, backend):
    # type: (Path, list[SimprintEntryMulti], str) -> None
    """Test search with detailed=False excludes chunk details."""
    index = SimprintMultiIndex(str(temp_index_path), backend=backend)
    index.add_raw_multi(sample_entries)

    results = index.search_raw_multi(
        {"CONTENT_TEXT_V0": [b"\x01" * 8]},
        limit=10,
        threshold=0.0,
        detailed=False,
    )

    assert len(results) > 0
    for result in results:
        for type_result in result.types.values():
            assert type_result.chunks is None
    index.close()


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_get_indexed_types(temp_index_path, sample_entries, backend):
    # type: (Path, list[SimprintEntryMulti], str) -> None
    """Test get_indexed_types returns sorted list."""
    index = SimprintMultiIndex(str(temp_index_path), backend=backend)
    assert index.get_indexed_types() == []

    index.add_raw_multi(sample_entries)
    types = index.get_indexed_types()

    assert types == ["CONTENT_TEXT_V0", "SEMANTIC_TEXT_V0"]
    index.close()


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_contains_existing_iscc_id(temp_index_path, sample_entries, backend):
    # type: (Path, list[SimprintEntryMulti], str) -> None
    """Test __contains__ with existing ISCC-ID."""
    index = SimprintMultiIndex(str(temp_index_path), backend=backend)
    index.add_raw_multi(sample_entries)

    assert sample_entries[0].iscc_id in index
    assert sample_entries[1].iscc_id in index
    index.close()


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_contains_non_existing_iscc_id(temp_index_path, sample_entries, backend):
    # type: (Path, list[SimprintEntryMulti], str) -> None
    """Test __contains__ with non-existing ISCC-ID."""
    index = SimprintMultiIndex(str(temp_index_path), backend=backend)
    index.add_raw_multi(sample_entries)

    non_existing = b"\x00\x10\xaa\xbb\xcc\xdd\xee\xff\x00\x11"
    assert non_existing not in index
    index.close()


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_contains_invalid_length(temp_index_path, backend):
    # type: (Path, str) -> None
    """Test __contains__ with invalid ISCC-ID length."""
    index = SimprintMultiIndex(str(temp_index_path), backend=backend)

    assert b"\x00\x10\x12" not in index  # Only 3 bytes
    index.close()


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_close_releases_resources(temp_index_path, sample_entries, backend):
    # type: (Path, list[SimprintEntryMulti], str) -> None
    """Test close releases all index resources."""
    index = SimprintMultiIndex(str(temp_index_path), backend=backend)
    index.add_raw_multi(sample_entries)

    assert len(index.indexes) == 2
    index.close()
    assert len(index.indexes) == 0


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_context_manager(temp_index_path, sample_entries, backend):
    # type: (Path, list[SimprintEntryMulti], str) -> None
    """Test context manager support."""
    with SimprintMultiIndex(str(temp_index_path), backend=backend) as index:
        index.add_raw_multi(sample_entries)
        assert len(index.indexes) == 2

    # Indexes should be closed after context
    assert len(index.indexes) == 0


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_reopen_index_loads_metadata(temp_index_path, sample_entries, backend):
    # type: (Path, list[SimprintEntryMulti], str) -> None
    """Test reopening index discovers existing sub-indexes and loads realm_id."""
    # Create and populate index
    index1 = SimprintMultiIndex(str(temp_index_path), backend=backend)
    index1.add_raw_multi(sample_entries)
    realm_id = index1.realm_id
    index1.close()

    # Reopen and verify - should auto-discover backend-specific files and load realm_id
    index2 = SimprintMultiIndex(str(temp_index_path), backend=backend)
    assert index2.realm_id == realm_id
    assert len(index2.indexes) == 2
    assert sample_entries[0].iscc_id in index2
    # Verify realm_id consistency across sub-indexes
    for idx in index2.indexes.values():
        assert idx.realm_id == realm_id
    index2.close()


def test_metadata_persistence_lmdb(temp_index_path, sample_entries):
    # type: (Path, list[SimprintEntryMulti]) -> None
    """Test that realm_id and ndim are persisted in LMDB sub-index metadata."""
    index = SimprintMultiIndex(str(temp_index_path), backend="lmdb")
    index.add_raw_multi(sample_entries)
    index.close()

    # Verify no JSON metadata file (coordinator is stateless)
    metadata_path = temp_index_path / "metadata.json"
    assert not metadata_path.exists()

    # Verify flat file structure exists
    assert (temp_index_path / "SIMPRINT_CONTENT_TEXT_V0.lmdb").exists()
    assert (temp_index_path / "SIMPRINT_SEMANTIC_TEXT_V0.lmdb").exists()

    # Reopen and verify sub-indexes have correct realm_id and ndim
    import lmdb

    for simprint_type in ["CONTENT_TEXT_V0", "SEMANTIC_TEXT_V0"]:
        subindex_path = temp_index_path / f"SIMPRINT_{simprint_type}.lmdb"
        env = lmdb.open(str(subindex_path), max_dbs=3, readonly=True, subdir=False)
        metadata_db = env.open_db(b"index_metadata")
        with env.begin() as txn:
            raw_data = txn.get(b"metadata", db=metadata_db)
            assert raw_data is not None
            metadata = json.loads(raw_data.decode("utf-8"))
            assert metadata["realm_id"] == "0010"
            assert metadata["ndim"] == 64  # From sample_entries
        env.close()


def test_metadata_persistence_lancedb(temp_index_path, sample_entries):
    # type: (Path, list[SimprintEntryMulti]) -> None
    """Test that realm_id and ndim are persisted in LanceDB sub-index metadata."""
    index = SimprintMultiIndex(str(temp_index_path), backend="lancedb")
    index.add_raw_multi(sample_entries)
    index.close()

    # Verify directory structure exists
    assert (temp_index_path / "SIMPRINT_CONTENT_TEXT_V0").is_dir()
    assert (temp_index_path / "SIMPRINT_SEMANTIC_TEXT_V0").is_dir()

    # Verify metadata files exist
    content_metadata = temp_index_path / "SIMPRINT_CONTENT_TEXT_V0" / "simprints_metadata.json"
    semantic_metadata = temp_index_path / "SIMPRINT_SEMANTIC_TEXT_V0" / "simprints_metadata.json"

    assert content_metadata.exists()
    assert semantic_metadata.exists()

    # Verify metadata content
    with open(content_metadata) as f:
        metadata = json.load(f)
        assert metadata["realm_id"] == "0010"
        assert metadata["ndim"] == 64  # From sample_entries


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_add_raw_multi_entry_without_simprints(temp_index_path, backend):
    # type: (Path, str) -> None
    """Test adding entry with empty simprints dict."""
    index = SimprintMultiIndex(str(temp_index_path), backend=backend)

    entry = SimprintEntryMulti(
        iscc_id=b"\x00\x10\x12\x34\x56\x78\x9a\xbc\xde\xf0",
        simprints={},
    )

    index.add_raw_multi([entry])
    assert index.realm_id == b"\x00\x10"
    assert len(index.indexes) == 0
    index.close()


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_search_result_ordering(temp_index_path, backend):
    # type: (Path, str) -> None
    """Test that search results are ordered by score descending."""
    index = SimprintMultiIndex(str(temp_index_path), backend=backend)

    # Create entries with varying simprint overlap
    entries = [
        SimprintEntryMulti(
            iscc_id=b"\x00\x10" + b"\x01" * 8,
            simprints={
                "CONTENT_TEXT_V0": [
                    SimprintRaw(simprint=b"\x01" * 8, offset=0, size=100),
                    SimprintRaw(simprint=b"\x02" * 8, offset=100, size=100),
                ]
            },
        ),
        SimprintEntryMulti(
            iscc_id=b"\x00\x10" + b"\x02" * 8,
            simprints={
                "CONTENT_TEXT_V0": [
                    SimprintRaw(simprint=b"\x01" * 8, offset=0, size=100),
                ]
            },
        ),
    ]

    index.add_raw_multi(entries)

    # Query with both simprints - first entry should score higher
    results = index.search_raw_multi(
        {"CONTENT_TEXT_V0": [b"\x01" * 8, b"\x02" * 8]},
        limit=10,
        threshold=0.0,
        detailed=False,
    )

    assert len(results) == 2
    assert results[0].score >= results[1].score
    index.close()


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_context_manager_with_exception(temp_index_path, sample_entries, backend):
    # type: (Path, list[SimprintEntryMulti], str) -> None
    """Test context manager properly closes even with exception."""
    try:
        with SimprintMultiIndex(str(temp_index_path), backend=backend) as index:
            index.add_raw_multi(sample_entries)
            raise ValueError("Test exception")
    except ValueError:
        pass

    # Index should be closed despite exception
    # We can't directly verify this, but the context manager should have called close()


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_load_metadata_with_null_realm_id(temp_index_path, backend):
    # type: (Path, str) -> None
    """Test opening empty index with no realm_id (no sub-indexes exist)."""
    # Create empty directory
    temp_index_path.mkdir(parents=True, exist_ok=True)

    # Open index - should load but realm_id remains None
    index = SimprintMultiIndex(str(temp_index_path), backend=backend)
    assert index.realm_id is None
    index.close()


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_load_metadata_with_empty_realm_id(temp_index_path, backend):
    # type: (Path, str) -> None
    """Test opening index with no sub-indexes (empty directory)."""
    # Create empty directory
    temp_index_path.mkdir(parents=True, exist_ok=True)

    # Open index - should load but realm_id remains None
    index = SimprintMultiIndex(str(temp_index_path), backend=backend)
    assert index.realm_id is None
    index.close()


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_validate_realm_id_when_none(temp_index_path, backend):
    # type: (Path, str) -> None
    """Test _validate_realm_id when realm_id is None (defensive check)."""
    index = SimprintMultiIndex(str(temp_index_path), backend=backend)
    assert index.realm_id is None

    # Should return early without error
    index._validate_realm_id(b"\x00\x10\x12\x34\x56\x78\x9a\xbc\xde\xf0")
    index.close()


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_validate_realm_id_invalid_length(temp_index_path, sample_entries, backend):
    # type: (Path, list[SimprintEntryMulti], str) -> None
    """Test _validate_realm_id with invalid ISCC-ID length after realm_id is set."""
    index = SimprintMultiIndex(str(temp_index_path), backend=backend)

    # Set realm_id first
    index.add_raw_multi([sample_entries[0]])
    assert index.realm_id is not None

    # Now try to validate an invalid length ISCC-ID
    with pytest.raises(ValueError, match="must be 10 bytes"):
        index._validate_realm_id(b"\x00\x10\x12")  # Only 3 bytes

    index.close()


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_get_or_create_index_without_entries(temp_index_path, backend):
    # type: (Path, str) -> None
    """Test _get_or_create_index when called without entries."""
    index = SimprintMultiIndex(str(temp_index_path), backend=backend)

    # Create index without providing entries (ndim=None)
    sub_index = index._get_or_create_index("TEST_TYPE_V0", entries=None)

    assert sub_index is not None
    assert "TEST_TYPE_V0" in index.indexes
    assert sub_index.ndim is None  # No auto-detection without entries

    index.close()


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_get_or_create_index_with_empty_entries_list(temp_index_path, backend):
    # type: (Path, str) -> None
    """Test _get_or_create_index with empty entries list."""
    index = SimprintMultiIndex(str(temp_index_path), backend=backend)

    # Create index with empty entries list
    sub_index = index._get_or_create_index("TEST_TYPE_V0", entries=[])

    assert sub_index is not None
    assert sub_index.ndim is None  # Can't auto-detect from empty list

    index.close()


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_get_or_create_index_with_entries_no_simprints(temp_index_path, backend):
    # type: (Path, str) -> None
    """Test _get_or_create_index when entries have no simprints."""
    index = SimprintMultiIndex(str(temp_index_path), backend=backend)

    # Create entries with empty simprints
    from iscc_search.indexes.simprint.models import SimprintEntryRaw

    entries = [SimprintEntryRaw(iscc_id_body=b"\x01" * 8, simprints=[])]

    sub_index = index._get_or_create_index("TEST_TYPE_V0", entries=entries)

    assert sub_index is not None
    assert sub_index.ndim is None  # Can't auto-detect without simprints

    index.close()


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_realm_id_int_property(temp_index_path, sample_entries, backend):
    # type: (Path, list[SimprintEntryMulti], str) -> None
    """Test realm_id_int property decodes realm correctly."""
    index = SimprintMultiIndex(str(temp_index_path), backend=backend)

    # Before adding data, realm_id_int should be None
    assert index.realm_id_int is None

    # Add entries to set realm_id
    index.add_raw_multi(sample_entries)

    # realm_id_int should now return decoded realm (0 for our test data)
    assert index.realm_id_int == 0

    index.close()


@pytest.mark.parametrize("backend", ["lmdb", "lancedb"])
def test_optimize(temp_index_path, sample_entries, backend):
    # type: (Path, list[SimprintEntryMulti], str) -> None
    """Test optimize() method calls optimize on all sub-indexes."""
    index = SimprintMultiIndex(str(temp_index_path), backend=backend)
    index.add_raw_multi(sample_entries)

    # Should not raise - optimize is called on all sub-indexes
    index.optimize()

    # Index should still work after optimization
    results = index.search_raw_multi(
        {"CONTENT_TEXT_V0": [b"\x01" * 8]},
        limit=10,
        threshold=0.0,
        detailed=True,
    )
    assert len(results) == 2

    index.close()
