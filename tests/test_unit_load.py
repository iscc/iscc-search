...
def test_load_from_file_restores_metadata(large_dataset):
    ...
    index1 = UnitIndex(max_dim=256, unit_type="META-NONE-V0", realm_id=7)
...
def test_load_uses_self_path(large_dataset):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Create and save
        index1 = UnitIndex(max_dim=256, path=path, unit_type="META-NONE-V0", realm_id=7)
        index1.add(iscc_ids[:10], iscc_units[:10])
        index1.save()
